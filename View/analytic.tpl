                ...
                <div id="hidden-pagination" style="border: 3px #ccc solid; width: 116px; margin-left: 219px;position: relative; display: none;">
                    <ul class="pagination" style="margin: 5px;">
                        {for $i=1 to $pagination['pagesCount']}
                            <li><a href="/main/analytic/{$category_id}/{$i}">{$i}</a></li>
                        {/for}
                    </ul>
                </div>
                <div>
                    <ul class="pagination" style="margin: 5px;">
                        <li><a href="/main/analytic/{$category_id}/1">1</a></li>
                        <li><a href="javascript: showPagination();">...</a></li>
                        <li><a href="/main/analytic/{$category_id}/{$pagination['pagesCount']}">{$pagination['pagesCount']}</a></li>
                    </ul>
                </div>

                <ul class="pagination" style="margin: 0px;">
                    <li>
                        <a {if ($pagination['prev'] != 0)} href="/main/analytic/{$category_id}/{$pagination['prev']}" {/if} aria-label="Previous">
                            <span aria-hidden="true">&laquo;</span>
                        </a>
                    </li>
                    <li><a>{$pagination['currentPage']}</a></li>
                    <li>
                        <a {if ($pagination['next'] != 0)} href="/main/analytic/{$category_id}/{$pagination['next']}" {/if} aria-label="Next">
