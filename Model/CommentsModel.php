...
class CommentsModel
{
    ...
    public function __construct($item_id, $user_name, $user_id, $rating, $comment, $parent_id, $moderation = 1)
    {
    ...
    public static function findById($id)
    {
        ...
        return $res ? new self($res['item_id'], $res['user_name'], $res['user_id'], $res['rating'], $res['comment'], $res['parent_id'], $res['moderation']) : null;
    }
    ...
    public static function getParentListByItemId($item_id = null, $order = "DESC", $lim = null)
    {
        $query = "SELECT * FROM `comments` ";
        if ($item_id) {
            $query .= " WHERE `item_id` = $item_id AND `parent_id` = 0 AND `moderation` = 1 ";
        }
        $query .= " ORDER BY `rating` $order, `id` $order";
        if ($lim) {
            $query .= " LIMIT $lim";
    ...
    public static function getListTopItemComments($lim = 3)
    {
        $query = "SELECT com.item_id as id, i.name as name, COUNT(com.id) as res FROM `comments` as com INNER JOIN items as i WHERE com.item_id = i.id  GROUP BY item_id ORDER BY res DESC";
        if ($lim) {
            $query .= " LIMIT $lim";
        }
    ...
    {
        $query = "SELECT c.`id`, c.`user_name`, c.`moderation`, c.`comment`, c.`date_time`, c.`item_id`, i.`name` FROM `comments` as `c` INNER JOIN `items` `i` where `c`.`item_id` = `i`.`id` ORDER BY c.`id` $order";
